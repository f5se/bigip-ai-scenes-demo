...
def _default_target() -> McpTarget:
    vs = MCP_INSIGHT_DEMO["default_vs"]
    assert isinstance(vs, dict)
    return McpTarget(host=str(vs["host"]), port=int(vs["port"]))


def _adapter_url(override: str | None = None) -> str:
    if override:
        return override
    return str(MCP_INSIGHT_DEMO.get("adapter_events_url", "http://127.0.0.1:8090/api/mcp-events"))


def _default_emit_audit() -> bool:
    """True = Demo Runner simulates F5 and POSTs Adapter; False = F5 iRule sends logs."""
    return bool(MCP_INSIGHT_DEMO.get("emit_audit_without_f5", True))


def _resolve_emit_audit(explicit: bool | None) -> bool:
    """F5 mode (emit_audit_without_f5=False) always disables Runner POST, even if client sends true."""
    if not _default_emit_audit():
        return False
...
def _audit_delivery_mode() -> str:
    return "runner" if _default_emit_audit() else "f5"
...
def _make_runner(
    ...
    *,
    ...
    return MCPClientRunner(
        ...
        agent,
        tenant,
        adapter_events_url=_adapter_url(adapter_url) if emit_audit else None,
...
async def mcp_insight_config() -> dict[str, Any]:
    ...
    payload["audit_delivery"] = _audit_delivery_mode()
    return payload
...
async def mcp_insight_health(
    ...
    adapter_base = _adapter_url().replace("/api/mcp-events", "")
    try:
        async with httpx.AsyncClient(timeout=3.0) as client:
...
async def mcp_insight_run(req: McpRunRequest) -> dict[str, Any]:
    target = req.target or _default_target()
    emit_audit = _resolve_emit_audit(req.emit_audit)
    runner = _make_runner(
        target, req.agent, req.tenant, emit_audit=emit_audit, adapter_url=req.adapter_url
    )
    try:
...
async def mcp_insight_run_stream(
    ...
    effective_emit_audit = _resolve_emit_audit(emit_audit)
    queue: asyncio.Queue[str | None] = asyncio.Queue()

    async def event_generator():
        runner = _make_runner(
            target, agent, tenant, emit_audit=effective_emit_audit, adapter_url=adapter_url
        )
        ...
        async def run_session() -> None:
            try:
                ...
                complete_payload = {
                    ...
                    "audit_delivery": _audit_delivery_mode(),
